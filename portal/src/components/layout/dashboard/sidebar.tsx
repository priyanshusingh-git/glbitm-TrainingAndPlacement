...
function NavContent({
  ...
  return (
    <div className="flex h-full w-full flex-col bg-brown-900 text-white select-none">
      {/* Brand Header */}
      <div className={cn("flex h-20 shrink-0 items-center transition-all duration-300", isSidebarCollapsed ? "justify-center" : "px-6")}>
        <Link 
          href={role === "student" ? "/student" : role === "trainer" ? "/trainer" : role === "recruiter" ? "/recruiter" : "/admin"} 
          className="flex items-center gap-3 overflow-hidden group"
          onClick={() => onLinkClick(role === "student" ? "/student" : role === "trainer" ? "/trainer" : role === "recruiter" ? "/recruiter" : "/admin")}
        >
          <div className={cn("relative shrink-0 transition-all duration-300", isSidebarCollapsed ? "h-10 w-10" : "h-10 w-10 ml-1")}>
            <Image 
              src="/glbitm-logo.png" 
              alt="GL Bajaj Logo" 
              fill
...
export function Sidebar({ role }: SidebarProps) {
  ...
  }, [expandOnHover, collapsed, setCollapsed])
  ...
  return (
    <>
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="w-72 border-r border-sidebar-border bg-brown-900 p-0">
          ...
          <NavContent 
            ...
            activeLoggingOut={activeLoggingOut}
      ...
      {/* Desktop Fixed Sidebar (Rock-solid fixed positioning) */}
      <aside
        onMouseEnter={expandOnHover ? handleMouseEnter : undefined}
        onMouseLeave={expandOnHover ? handleMouseLeave : undefined}
        className={cn(
          "fixed inset-y-0 left-0 z-40 hidden h-dvh max-h-screen flex-col border-r border-white/15 bg-brown-900 shadow-2xl transition-all duration-300 md:flex",
          collapsed ? "w-16" : "w-64"
        )}
      >
        <NavContent 
