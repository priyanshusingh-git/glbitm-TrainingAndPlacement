...
export const updateStudent = async (id: string, body: any, performedById: string, context?: { ipAddress?: string; userAgent?: string }) => {
  ...
  if (Object.keys(profileData).length > 0) {
    ...
    // Check if isProfileLocked explicitly changed
    if (typeof isProfileLocked === 'boolean' && isProfileLocked !== profile.isProfileLocked) {
      ...
      const notification = await prisma.notification.create({
        data: {
          ...
          message: isProfileLocked
            ? "Your entire profile has been locked by admin. You can no longer make changes."
            : "Your profile has been unlocked by admin. You can now make changes.",
          type: isProfileLocked ? 'WARNING' : 'SUCCESS'
        }
      });

      // Synchronized Ably Broadcast (Unified channel student-{userId} and event data-update)
      try {
        await broadcastMessage({
          channel: `student-${userId}`,
          event: 'data-update',
          payload: { type: isProfileLocked ? 'PROFILE_LOCKED' : 'PROFILE_UNLOCKED' }
        });
        ...
        logger.warn('Realtime broadcast failed during profile status change:', broadcastError);
      }
    } else {
      // General info update
      await logAudit({
      ...
      if (updateFields.length > 0) {
        ...
        try {
          ...
          logger.warn('Realtime broadcast failed during profile data update:', broadcastError);
        }
      }
    }
  }
...
export const deleteStudent = async (id: string) => {
  ...
  // 2. User Table Delete (Postgres is sole authority)
