...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // For each recruiter, find their linked company by email match
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // If companyId provided, verify it exists and update its email to match
    if (companyId) {
      const company = await prisma.company.findUnique({ where: { id: companyId } })
      if (!company) {
        return NextResponse.json({ error: 'Company not found' }, { status: 404 })
      }
      // Link: set the company email to match the recruiter email
    ...
    const recruiter = await prisma.user.create({
      data: {
        ...
        name,
        email,
        password: 'FIREBASE_AUTH', // Match exact pattern required
        role: 'RECRUITER',
        mustChangePassword: true,
      },
    })
    ...
    return NextResponse.json({
      ...
      warning: !emailSent
        ? 'Recruiter created but welcome email failed. Share credentials manually.'
        : undefined,
      emailSent,
    }, { status: 201 })
    ...
    if (firebaseUserId) {
      try {
        ...
        logger.error('Rollback failed:', cleanupError)
      }
    }
